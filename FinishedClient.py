...
class Client:
    ...
    def task(self):
        ...
        if serv_mess:
            ...
            if isinstance(serv_mess, HamzaIntroMessage):
                enc_type, key = serv_mess.unpackIntro()
                enc_type, key = FinishedSecurity.decode_custom(enc_type), FinishedSecurity.decode_custom(key)
            elif isinstance(serv_mess, HamzaMessage):
                recv_user, recv_mess = serv_mess.unpack()
                recv_mess = FinishedSecurity.decode_random(enc_type, recv_mess, key)
        ...
        if self.mess_is_ready_to_send is True:
            ...
            mess_obj_intro = HamzaIntroMessage(enc_type, key)
            out_mess_intro = pickle.dumps(mess_obj_intro)
            mess_obj = HamzaMessage(user, secretmessage)
            out_mess = pickle.dumps(mess_obj)
            self.sock.send(out_mess_intro)
            time.sleep(100)
